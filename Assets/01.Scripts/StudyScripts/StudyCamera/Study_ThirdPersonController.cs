...
public class Study_ThirdPersonController : MonoBehaviour
{
    ...
    private void UpdateRotation()
    {
        ...
        {
            headPivotTransform.Rotate(Vector3.up * mouseX);
        }
        else
        {
       ...
       // cameraTransform.Rotate(Vector3.right * -mouseY);
        headPivotTransform.Rotate(Vector3.right * -mouseY);
        // Y축 회전은 플레이어의 회전에 반영하되,
        // X축 회전은 카메라 회적에 반영을 해야합니다.
        
        
       
        headPivotTransform.transform.rotation = Quaternion.Euler(angleX, 0f, 0f);
